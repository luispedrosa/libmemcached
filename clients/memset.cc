...
#define PROGRAM_NAME "memset"
#define PROGRAM_DESCRIPTION "Set a a key value."
...
int main(int argc, char *argv[])
{
  ...
  while (optind < argc)
  {
    char *key = strtok(argv[optind], "=");
    char *value = strtok(NULL, "=");
    assert(key && value && "Error parsing key=value.");
    ...
    if (opt_verbose)
    {
      ...
      printf("op: %s\nkey: %s\nkey length: %lu\n"
	     "value: %s\nvalue length: %lu\nflags: %x\nexpires: %lu\n",
	     opstr[opt_method - OPT_SET], key, strlen(key),
	     value, strlen(value), opt_flags, (unsigned long)opt_expires);
    }
